...
function get_all_hierarchies(){
	...
	$location_data = $db_obj->get_results("SELECT DISTINCT Id_Ort FROM orte WHERE orte.Id_Kategorie = $category AND NOT EXISTS(SELECT * FROM Polygone_Vereinfacht p WHERE p.Id_ort = orte.id_ort AND Epsilon = 0.0006) and Id_ort not in (1390160, 1390476, 1390904, 1390924, 1391002, 1396772, 1397332, 1399088, 1399240, 1399578, 1400276, 1400912, 1401154, 1401176, 1409656)");
